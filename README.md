...
GUI 沿用 `~/.pi`，提供真实会话、工具、审批、模型认证、会话树和业务面板。Web 免登录，仅用于可信局域网。参数及默认值见 `opi-web --help`，访问地址以启动输出为准。只在本机使用时加 `--host 127.0.0.1`。桌面产物在 `dist/desktop/release/`，MVP 尚未签名。功能边界、手机访问和验证方法见 [GUI MVP](docs/gui.md)。
