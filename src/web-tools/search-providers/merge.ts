...
function toOutput(candidate: Candidate, rank: number): WebSearchItem {
	return { rank, title: candidate.title, url: candidate.url, ...(candidate.snippet !== undefined ? { snippet: candidate.snippet } : {}), provenance: [...candidate.providers].map(([provider, providerRank]) => ({ provider, rank: providerRank })) };
}
