...
export async function runBuiltin(host: GuiExecution, text: string, execute: (action: GuiAction) => Promise<void>): Promise<boolean> {
	...
	switch (name) {
		case "new":
			...
			break;
		case "resume":
			await execute(args ? { action: "openSession", path: path.resolve(host.runtime.cwd, args) } : { action: "sessions" });
