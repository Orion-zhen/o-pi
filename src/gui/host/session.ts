...
import type { SessionManager, AgentSessionRuntime, SessionStartEvent } from "@earendil-works/pi-coding-agent";
...
import type { GuiAction, GuiEvent, GuiQuery, GuiQueryResults, GuiSnapshot } from "../contract.ts";
...
export class GuiSession {
	...
	constructor(readonly id: string, private changed: () => void, private refreshSessions: () => void) {}
	...
	subscribe(listener: Listener): () => void {
		...
		return () => {
			...
			if (!this.listeners.size) this.info.invalidate();
		};
	}
			...
			try {
				const runtime = await createGuiRuntime(manager.getCwd(), {
					...
					trackBackground: (task, cancel) => {
						...
						void this.track(task).finally(() => this.background.delete(cancel)).catch((error: unknown) => this.dialogs.notify(String(error), "error"));
					},
	...
	private scheduleInfo(): void {
		if (!this.disposed && !this.changing && this.current && this.readSessionInfo && this.listeners.size)
	...
	dispatch(action: GuiAction, client: SessionClient): Promise<void> {
		...
		return this.track(this.origin.run(client, () => this.perform(action)));
	}
	query<Q extends GuiQuery>(query: Q): Promise<GuiQueryResults[Q["query"]]>;
	query(query: GuiQuery): Promise<GuiQueryResults[GuiQuery["query"]]> {
		if (this.disposed) return Promise.reject(new Error("会话实例已释放。"));
	...
	private readGit(cwd: string, signal: AbortSignal): Promise<WorkspaceGit | null> {
		...
		const pending = readWorkspaceGit(cwd, signal).finally(() => { if (this.pendingGit === pending) this.pendingGit = undefined; });
		this.pendingGit = pending;
		return pending;
	}
	private async readQuery(query: GuiQuery): Promise<GuiQueryResults[GuiQuery["query"]]> {
		...
		switch (query.query) {
			...
			case "previewFile": {
				...
				const result = query.query === "workspaceFiles" ? await listWorkspaceFiles(query.cwd, query.path)
					: query.query === "workspaceGit" ? await this.readGit(query.cwd, signal)
					: await previewWorkspaceFile(query.cwd, query.path, signal, () => this.readGit(query.cwd, signal));
				signal.throwIfAborted();
				return result;
			}
			default: throw new Error("非会话查询。");
		}
	}
	...
	private async perform(action: GuiAction): Promise<void> {
		switch (action.action) {
			...
			case "draft": this.origin.getStore()?.writeDraft(action.text); return;
	...
	private async prompt(action: Extract<GuiAction, { action: "prompt" }>): Promise<void> {
		...
		this.publish();
		const client = this.origin.getStore();
		if (!client) throw new Error("缺少发起客户端。");
	...
	async dispose(): Promise<void> {
		...
		this.loginController?.abort(); this.commandController.abort(); this.workbenchController.abort(); this.dialogs.cancel();
		if (this.current) {
