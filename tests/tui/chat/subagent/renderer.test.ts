...
describe("subagent renderer", () => {
	...
	it("展开态展示 running subagent 的实时事件", () => {
		const details: SubagentDetails = {
			...
			results: [
				runningResult({
					...
					events: [
						{ type: "tool", name: "read", args: { path: "src/tui/chat/subagent/renderer.ts" } },
		...
		for (const value of ["scout", "read", "src/tui/chat/subagent/renderer.ts", "found renderer behavior"]) {
