...
export async function exerciseModels(page: Page, settingsFile: string, screenshotName: string) {
	...
	await expect(modelSelect.locator("option:checked")).toContainText("未加入已选模型");
	await expect(modelSelect.locator("option:checked")).toBeDisabled();
	...
	await expect.poll(choices).toEqual([]);
	await close();
	await expect(modelSelect).toBeDisabled();
	await expect(modelSelect.locator("option:checked")).toContainText("GUI Second Model · 请在侧栏选择模型");
	await open();
	...
	await expect.poll(choices).toEqual(initial);
	await panel.getByRole("button", { name: "使用模型 gui-test/test", exact: true }).click();
