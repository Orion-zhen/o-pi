...
async function generationFromSources(root: string, sources: ReadonlyMap<string, string>): Promise<RepoMapGeneration> {
	...
	const metadata: RepoMapMetadata = {
		...
		tooLargeFileCount: 0, diagnosticCount: architecture.diagnostics.length, configFingerprint: "config", ignoreFingerprint: "ignore",
	};
...
function graphGeneration(): RepoMapGeneration {
	...
	const metadata: RepoMapMetadata = {
		mapId: "a".repeat(64), repositoryRoot: temp.path, worktreeRoot: temp.path, gitCommonDir: path.join(temp.path, ".git"), generation: "b".repeat(64),
		createdAt: "2026-07-18T00:00:00.000Z", updatedAt: "2026-07-18T00:00:00.000Z", freshness: "fresh",
		...
		configFingerprint: "config", ignoreFingerprint: "ignore",
	};
...
function symbol(file: RepoMapFileRecord, name: string): RepoMapSymbolNode {
	return { kind: "symbol", id: `symbol:${name}`, fileId: file.id, symbolKind: "function", name, qualifiedName: name, signature: `function ${name}()`, exported: false, startLine: 1, endLine: 1, startByte: 0, endByte: 1, definitions: [name], references: [], calls: [] };
}
