...
port.on("message", (request) => {
	const results = request.paths.map((filePath) => parser.analyzeCodeFile(filePath, request.text));
	port.postMessage(results.map((result) => ({ units: result.index.units.length, imports: result.imports.length })));
});
