...
工具只依赖 `WorkspaceFileSystem` 提供的分组能力：`paths`、`metadata`、`content`、`visibility`、`discovery` 和 `mutations`。路径解析返回不透明引用，后续操作不再接受原生路径。每次调用都会绑定工作区身份、不可变策略、调用级可见性状态和操作上下文。

`discovery` 内部统一处理目录遍历、相对范围的 glob、静态前缀剪枝、原始范围深度、明确指定的软忽略根目录或静态前缀，以及子符号链接。`discoverPaths` 为 `find` 提供不额外读取普通条目元数据的路径模式。`discover` 为 `grep` 提供包含对象身份、版本和大小快照的完整模式。两种模式共用单次流、排序、扫描预算和取消逻辑。

内容读取的 `expectedSnapshot` 选项要求打开的文件与调用方捕获的版本一致。读取结束时还会复验文件是否变化。完整正文读取与修改快照读取共用稳定读取实现，正常结果、已知失败和未知异常都关闭句柄。
...
`read` 可以在当前会话记录原始字节版本。成功的 `write` 和 `edit` 通过文件系统提交回调记录写入后的版本，因此可以直接执行 `write → edit`。观测状态以文件系统规范身份为键，明确指定的符号链接与目标共享版本身份。当前会话发起 `bash` 时，系统在命令前后稳定读取已观察文件。只有命令开始时仍等于观测版本的文件才会在命令结束后更新版本。`edit` 在目标专属队列中读取当前快照，再校验观测状态，避免覆盖排队期间或未被当前会话采纳的修改。
