...
`FileRef`、`DirectoryRef` 和 `TargetRef` 使用对象身份。命名空间通过私有 `WeakMap` 保存引用对应的原生身份，不生成字符串路径 ID。复制、伪造或来自其他命名空间的引用不能通过归属检查。原生桥接器供 LSP 适配器和会话观测映射使用，工具命令不能从引用中取回原生路径。LSP 只能通过命令提供的快照绑定加载器读取允许清单中的正文。
