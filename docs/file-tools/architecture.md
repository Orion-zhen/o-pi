...
4. 主机返回同一份调用对象，包含 `filesystem`、`limits`、`observation`、`operation` 和原生桥接器。命令直接使用所需字段和配置中的预算名称，不再逐工具改名。租约由文件系统运行时统一跟踪，主机不维护第二份租约集合。原生桥接器只供组合适配器使用。
...
| 所有者 | 持有状态 | 释放行为 |
| --- | --- | --- |
| 扩展 | 已加载的 `grep` 适配器、LSP 模块加载 Promise、主机 | 关闭时先停止新调用，再释放已经加载的对象 |
