...
Pi 扩展入口位于 `src/extensions/file-tools.ts`。六个工具分别位于 `src/file-tools/{ls,read,write,edit,find,grep}/`，互不导入。工具之间共享错误、诊断、差异契约和搜索截断导航。`src/filesystem/` 是工作区 I/O 的唯一数据平面，不依赖 Pi、模型输出、LSP、Tree-sitter 或具体工具结果。
