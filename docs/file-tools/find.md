...
filesystem path discovery 先应用 scope、glob、blocked path、soft ignore、深度和 symlink 规则。`readdir` 已分类的普通文件和目录使用已验证父目录 identity 投影，不为每个 entry 读取 metadata 或执行 `realpath`；symlink 和未知类型仍经过 namespace 解析。目录读取按小批次预取并保持确定性输出顺序，后续明确读取会重新解析并验证路径。

visibility 在同一次目录枚举上发现并增量编译嵌套 `.gitignore` / `.piignore`，不在 find 前预扫描整棵目录树。若一个 ignored 目录可能被尚未发现的嵌套高优先级规则重新包含，runtime 只检查该 ignored 子树的规则文件后再决定能否安全剪枝。
...
query term 的 Unicode/case 形式每次调用只编译一次。每个唯一候选在 discovery 产出时立即评分；runtime 继续遍历并统计全部命中，但只维护 `find_result_limit` 大小的 relevance 前缀，不物化完整候选数组，也不对 limit 之外的命中执行全量排序。
