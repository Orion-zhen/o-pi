...
文件系统路径发现能力先应用搜索范围、glob、受阻路径、软忽略规则、深度限制和符号链接规则。系统从已验证的父目录身份投影 `readdir` 已分类的普通文件和目录条目，不为每个条目读取元数据或执行 `realpath`。符号链接和未知类型仍会经过命名空间解析。每个搜索范围采用广度优先遍历，先处理浅层条目，再进入更深层目录。每个目录内按名称稳定排序，待访问目录按小批次预取。后续明确读取路径时，系统会重新解析并验证路径。
...
系统在每次调用中只编译一次查询词的 Unicode 和大小写形式。路径发现每产生一个唯一候选，系统就立即评分。运行时继续遍历并统计全部命中，只维护不超过 `find_result_limit` 的相关性前缀，以及每个正向 OR 分支的最佳候选。系统不会构造完整候选数组，也不会对限制外的命中执行全量排序。

命中数超过结果限制时，前 4 条相关性结果保持不变。其余名额先按最佳候选的相关性覆盖尚未展示的正向 OR 分支，再按原始相关性填满。否定分支不参与覆盖，所有候选仍必须满足完整查询。名额不足时不保证覆盖全部分支，结果限制不超过 4 时保持纯相关性前缀。未发生结果截断时不改变排序。

运行时不接受 `case`、`exact`、`kind`、`field`、`scheme`、`sort` 或 `tiebreak` 参数。文件和目录统一搜索，始终启用路径匹配模式和智能大小写。

## 输出

模型正文按上述选择顺序直接返回路径。目录以 `/` 结尾。结果不会折叠目录、混入附近的非命中项或显示分数：
...
- `entry_limit`：所有范围共享的遍历量达到 `find_max_entries`。未匹配查询或 glob 的条目也消耗扫描预算。
...
`findFiles` 使用主机租约组合后的调用与关闭信号，不单独维护工具实例。路径发现会关闭迭代器，流式排名分批让出事件循环并检查取消。`find` 不持有工作线程、索引或跨调用排名缓存。
