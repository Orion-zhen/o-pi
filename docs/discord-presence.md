...
并行执行工具时，最后启动且尚未结束的工具决定当前活动。补充流式参数只更新该工具的元数据，不改变启动顺序。该工具结束后，扩展会恢复到仍在运行的上一个工具。所有工具结束后，如果当前轮次尚未结束，则恢复为 `thinking`。`agent_settled` 后切换为 `idle`。
...
| `/presence reload` | 重新读取默认、用户和项目配置。保留当前工具活动和运行时开关覆盖，并在原档位仍存在时保留档位覆盖。 |
| `/presence profile <name>` | 临时切换到内置或用户定义的展示档位。命令补全读取当前配置。 |

`on`、`off` 和档位选择不会写入配置。新的 Pi 会话重新使用配置中的 `enabled` 和 `profile`。

输出管理器把目标应用和活动内容作为一个整体更新，合并快速状态变化，并按 `update_interval_ms` 限制发送尝试。如果状态在发送前恢复为已经展示的内容，中间状态会被丢弃。新的协调进程没有前次发送时，首个状态会立即发送。连接正常时，相同载荷不会重复发送。Discord 不可用时，Pi 不等待连接完成，发送器会按 `retry_interval_ms` 重试最新状态。
...
- `activity.ts` 分类工具和提取展示元数据，`streaming.ts` 只负责 JSON 字符串边界识别，`render.ts` 渲染展示内容。
- `coordinator-client.ts` 管理单个客户端连接任务，`coordinator-server.ts` 选择最近活跃的参与者。
- `output.ts` 统一管理目标应用、最新内容、限频、重试和连接切换，`transport.ts` 隔离 Discord RPC 库。
- 输出的 `hide()` 允许后续恢复展示，`dispose()` 永久释放资源。服务器负责最终销毁输出，守护进程负责端点与锁文件清理。
