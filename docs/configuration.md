...
加载器会解析所有层，并根据模块规则在合并前或合并后执行 JSON Schema 与语义校验。配置文件可以带 UTF-8 BOM。非法 JSONC、未知字段、错误类型或无效语义都会导致配置被拒绝，不会静默跳过错误层或回退到较低层。各模块向调用方呈现配置错误的方式可能不同。例如，文件工具返回 `CONFIG_ERROR`。

为避免读取到并发写入产生的混合内容，加载器会在读取前后比较所有配置文件的指纹。文件发生变化时，加载器最多重新读取两次。三次读取都无法获得稳定快照时，配置加载失败。

文件工具和网页工具共用 `src/harness/config-cache.ts`，按文件路径与指纹合并并发加载并缓存快照。返回值是调用方独占的副本，修改它不会影响缓存或其他调用。文件工具关闭时清理缓存，关闭前开始的加载不会重新填充已经清理的缓存。

工具启用配置在 Schema 校验后只编译匹配规则，不再重复检查字段的基本类型。子代理先合并原始字段，最后只转换一次运行时配置。各模块仍保留专属合并规则和跨字段语义校验。
