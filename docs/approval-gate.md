...
命令、文件内容、路径、工作目录和触发原因在各前端展示时都会移除终端控制序列，原始工具参数不被修改。TUI 和 RPC 使用同一份文本内容。RPC 模式使用 Pi 的基础选择框，并在标题中包含请求信息。

Web 与 Desktop 注入 GUI 呈现器。Bash 命令、工作目录和敏感单元按结构化数据传入原审批弹窗，不解析标题文本。命令语法高亮，敏感单元及原因分层展示。长命令默认预览前 240 个 Unicode 字符、最多 4 行，使用现有 shadcn 组件展开与收起。敏感操作列表保持独立，审批选项、超时和拒绝行为不变。语法颜色只辅助阅读，不参与策略判断。
...
存储通过 `FileApprovalStore.open` 完成初始化后才能使用。GUI 宿主通过 `ApprovalStores` 为同一路径共享持久规则和写入队列，各逻辑会话使用独立的 `SessionApprovalRules`。实例回收不丢失临时授权，新会话和分支不继承，宿主退出后临时授权失效。

同一路径的并发调用等待同一次加载，加载失败后允许下一次调用重试。判定前检查文件身份及修改信息，读取外部变更后的持久规则。写入串行提交并原子替换，保存失败不更新内存规则，也不阻塞后续写入。读取损坏或不可读的规则文件时阻止执行。该队列只协调同一宿主，不保证其他 TUI 进程同时写入时不会覆盖规则。
