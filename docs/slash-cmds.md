...
本页记录 `src/harness/extensions/` 提供的斜杠命令，终端视图由 `src/tui/extensions.ts` 装配。除 `/skill:<name>` 外，命令都通过 `pi.registerCommand()` 注册。注册时命令名不含 `/`，在 Pi 输入框中以 `/命令名` 调用。`/skill:<name>` 的候选项来自 Pi 的技能发现，由本扩展的输入钩子接管。模型和思考级别分别使用 Pi 原生的 `/model` 与 `/thinking`。`thinking-preferences` 扩展会在当前会话分支内按模型记忆原生 `/thinking` 选择。
...
来源：`src/harness/extensions/system-prompt.ts`
...
来源：`src/harness/extensions/skill-context.ts`
...
来源：`src/harness/extensions/stats.ts`
...
- 会话总用量包含缓存保温、压缩摘要、分支摘要和工具自身报告的用量。缓存保温另列次数、词元和费用，已包含在总量内。最近一轮、平均每轮和缓存命中率只统计对话中的助手请求。
- 成本只显示为 `est`，不代表账单。
...
来源：`src/harness/extensions/prune.ts`
...
- 从有效会话消息中估算系统指令、工具声明及其变更，不再额外累计静态提示词。模型不支持中途系统消息时，先使用 Pi 的重放逻辑合并系统消息。
...
来源：`src/harness/extensions/telemetry.ts`
...
来源：`src/harness/extensions/usage.ts`
...
来源：`src/harness/extensions/discord-presence.ts`
...
来源：`src/harness/extensions/subagent.ts`
