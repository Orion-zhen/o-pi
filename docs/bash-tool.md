...
这些规则只提供轻量防护。`bash` 工具不解析 Bash 抽象语法树，不限制网络，不修改 `HOME` 或工作目录，也不限制 Shell 语法。工具对执行环境的修改仅包括 Pi 托管的 `bin` 目录、当前 `PI_*` 变量、Python 虚拟环境变量和上述 Windows 路径兼容处理。
