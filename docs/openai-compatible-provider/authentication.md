...
扩展比较请求头名称时不区分大小写。提供方配置和模型配置都不会覆盖调用方显式设置的请求头，即使调用方的值恰好与提供方相同。调用方可以用 `null` 禁用同名请求头。
...
认证解析把无密钥标记和已解析的提供方请求头放入请求级 `env`。流式请求在传输边界确定默认 `Authorization` 是否禁用，再按提供方、模型、调用方的顺序一次合并请求头。提供方请求头不提前放入 `auth.headers`，以免 Pi 合并后丢失调用方覆盖信息。

模型目录端点使用 Pi 联网刷新阶段传入的 API key credential，从其中的 `env` 读取同一份认证信息，不重新执行命令或解析提供方配置。
