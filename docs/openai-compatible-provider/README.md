...
即使配置了手写模型，联网刷新也会请求模型目录端点，并用远端数据补充模型目录。`modelsEndpoint` 可以指定相对路径或完整 URL。端点必须返回 `{ "data": [...] }`。`data` 中的每个对象必须包含非空且不重复的 `id`。

扩展只读取 `context_length` 和 `architecture.input_modalities`。扩展保留手写模型的顺序和字段。远端元数据只补充缺失的上下文窗口和图片输入能力，远端独有模型追加到目录末尾。

Pi 的 `ModelsStore` 会按提供方 ID 保存刷新后的模型目录。离线模式先使用手写模型，再恢复该提供方的缓存覆盖；联网刷新成功后由 Pi 原生 Provider 生命周期替换缓存。完整流程见[自动发现](discovery.md)。
...
- 模型目录缓存遵循 Pi 原生 Provider/`ModelsStore` 生命周期，只包含模型目录和刷新时间，不包含 API 密钥、认证请求头或扩展内部标记。
...
确认模型目录端点返回包含 `data` 数组的对象，且每个条目是包含非空 `id` 的对象。然后检查 HTTP 状态、认证配置和 `modelsEndpoint`。自动发现使用独立的 30 秒超时。提供方的 `timeoutMs` 控制模型流式请求，不控制模型目录请求。
