...
数学模块不进入启动加载的关键路径。启动完成或 `agent_settled` 后连续空闲 750ms，且没有排队消息时，才尝试初始化。支持 Kitty 或 iTerm2 图片协议时预热 MathJax 字体。未就绪、字体失败或公式无法渲染时，继续使用 Pi 原生文本渲染。字体预热失败不会永久缓存 Promise，后续空闲初始化可以再次尝试。旧会话任务不得更新新会话界面。
