...
`skill-context` 提供三项能力：参数结构固定的 `skill` 工具、手动 `/skill:<name>` 加载，以及供 `read` 和 `bash` 使用的 `skill://` 资源定位符。
...
加载技能后，模型可以使用 `read` 读取技能目录中的相关资源：
...
`skill://` 是逻辑资源定位符，不是技能名称或操作系统路径。`read` 可以读取定位符指向的资源。仅当定位符是一个完整且可静态确定的参数时，`bash` 才会将定位符解析为真实路径。模型可以通过这种方式运行技能附带的脚本。定位符不能传给 `write` 或 `edit`。定位符误传给 `skill` 时，工具会返回改用 `read` 的提示。
...
`bash` 支持未引用、单引号和双引号参数。工具拒绝动态拼接的定位符，也拒绝只出现在 `bash -c` 内嵌脚本中的定位符。路径解析不会把技能目录变成只读文件系统。命令仍使用 `bash` 工具原有的进程权限。
...
- `read` 返回的路径字段和遥测使用逻辑 URI，不包含解析后的真实路径。`bash` 不会从命令输出中删除真实路径。
...
解析 `skill://` 定位符时，扩展直接使用加载阶段已规范化的逻辑根，只解析目标的真实路径。`read` 和 `bash` 共用授权检查和路径边界检查逻辑。执行 `/reload` 后，新扩展实例会重建候选索引和路径缓存。
