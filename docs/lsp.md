...
| `max_items` | `8` | `write` 和 `edit` 返回给模型和展开后终端界面的诊断条数上限，范围为 `0`-`100`。统计字段仍按文件中的全部诊断计算。 |
...
- `edit`：预览不调用 LSP。成功写盘后，Pi 发送受监视文件的更改事件，展示当前文件的错误总数和有界错误清单。只使用同一工作区和服务器来源的编辑前基线区分 `new` 和 `existing`，新增错误优先。来源不同或未知的基线标记为 `unknown`，仍展示当前错误，但不猜测新增或已存在。警告只展示修改范围内的新增问题。不再为诊断归因请求文档符号。诊断失败不会改变 `status: "applied"`。
- `ls` 和 `find`：不接入 LSP。

`edit` 和 `write` 都不在模型正文或工具 TUI 中报告 `clean` 和已修复计数。没有可见诊断时只确认文件修改成功。诊断超时或不可用仍明确标记，不能当作错误已经修复。`write` 同样优先展示当前错误及其 `new` / `existing` 标记，随后展示警告。

LSP 增强不会自动应用代码操作、整理导入或执行跨文件重命名。

### 关系导航、修复提示与关联诊断

- `grep` 复用传入调用和引用响应，为前 4 个输出区域各附加最多 2 个关系位置。实际调用点优先，位置去重并通过搜索范围和快照绑定加载器校验。每个代码单元最多探测 16 个候选，不发送额外 LSP 请求。
- `edit` 和 `write` 仅对可见错误请求 `textDocument/codeAction`。每个文件最多请求 3 次，同一客户端的整批提示共用不超过 300 ms 的等待预算，并受 `diagnostics.max_wait_ms` 限制。已知基线中的原有错误不请求提示，未知基线沿用诊断的不确定归因。
- 提示只接受唯一的、已经带有内联编辑的单文件 `quickfix`，最多 4 处替换，每处新文本不超过 2048 个 UTF-16 代码单元。禁用操作、命令、延迟解析、跨文件操作和多个候选均不提供提示。只展示不超过 160 个 Unicode 码点的标题，不应用编辑或调用 `codeAction/resolve`、`workspace/executeCommand`。提示失败不丢弃已有诊断。
- 修改前捕获当前文件和最近 32 个同源文件的诊断基线。写后只消费本次拉取报告中的完整 `relatedDocuments` 报告，不建立依赖图或额外发出工作区诊断请求。基线已知时只报告新增错误，否则标记 `causality uncertain`。新增仅表示与基线相比出现，不证明唯一因果关系。
- 关联诊断最多展示 3 个文件，与当前文件共用 `diagnostics.max_items` 上限。同批次已修改的文件不重复列为关联文件，其他关联文件在同一客户端批次内去重。文件工具绑定层再次检查受阻路径、忽略规则和符号链接边界。没有关联报告不代表其他文件没有错误。
...
`grep` 分析器要求服务器同时支持文档符号、引用和传入调用。没有正文命中时，服务器还必须支持工作区符号。客户端为生产所需请求提供带超时与协议级取消的类型化接口，包括诊断后的代码操作提示请求，并接收诊断通知；不声明或消费 work-done progress，也不提供通用通知转发。
