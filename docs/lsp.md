...
initialize 返回的 capabilities 会保存在 session 中；不支持的 document symbols、workspace symbols 或 references 不会发送请求。session 提供带超时和协议级取消的 typed request/notification 入口，并统一接收 diagnostics、日志和 progress。
