...
Pi 负责解析模板、替换参数和处理同名冲突。本仓库的 `src/harness/extensions/agents-prompts.ts` 只通过 `resources_discover` 增加 `.agents/prompts` 中的模板文件。
...
`src/harness/extensions/agents-prompts.ts` 按以下规则发现模板：
